...
def ensure_data_exists():
    """
    Checks if data exists. If not, runs ingestion.
    NOT cached, so we control the UI logic explicitly.
    """
    base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    data_dir = os.path.join(base_dir, 'data')
    
    DATA_VERSION = 2 
    version_file = os.path.join(data_dir, f'.version_{DATA_VERSION}')
    
    data_exists = os.path.exists(data_dir) and any(f.endswith('.parquet') for f in os.listdir(data_dir))
    version_match = os.path.exists(version_file)
    
    if not data_exists or not version_match:
        status_placeholder = st.empty()
        
        if not data_exists:
            msg = "⚠️ No data found (Cloud Deployment). Generating seed data..."
        else:
            msg = "⚠️ Data schema updated. Regenerating dataset..."
            
        status_placeholder.warning(msg)
        
        try:
            # Clean old data if version mismatch
            import shutil
            if os.path.exists(data_dir):
                shutil.rmtree(data_dir) # Wipe old data
            
            # Re-import to ensure we get fresh code if reloaded
            sys.path.append(os.path.join(base_dir, 'src'))
            sys.path.append(os.path.join(base_dir, 'src', 'ingestion'))
            
            from ingestion.ingestor import run_ingestion_pipeline
            run_ingestion_pipeline(num_records=5000)
            
            # Create version file
            with open(version_file, 'w') as f:
                f.write(str(DATA_VERSION))
            
            status_placeholder.success("✅ Data generated successfully!")
            time.sleep(2) 
